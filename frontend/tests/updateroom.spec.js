...
test('update room', async ({ page }) => {
  ...
  await roomRow.locator('[data-testid^="edit-room-button-"]').click();
  await expect(page.getByText('Update Room')).toBeVisible();

  const updatedPrice = '90';
  await page.getByTestId('updating-room-price-input').fill(updatedPrice);
  await page.getByTestId('update-room-submit-button').click();

  await expect(page.getByText('Room updated successfully!')).toBeVisible();
  await expect(roomRow).toContainText(`$${updatedPrice}`);
});
